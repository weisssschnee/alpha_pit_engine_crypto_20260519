...
$runtimeRoot = Join-Path $Repo "runtime\a7ff19_company_parallel"
$queuePath = Join-Path $Repo "runtime\a7ff19_external_selector_confirmation_contract\a7ff19_execution_queue.csv"
...
if (-not (Test-Path $queuePath)) {
  throw "A7FF-19 queue not found: $queuePath"
}
...
for ($i = $StartShard; $i -lt ($StartShard + $ShardCount); $i++) {
  ...
  $offset = $i * $ShardSize
  $stage = "A7FF-19S$sid"
  $filePrefix = "a7ff19s$sid"
  $runtime = Join-Path $Repo "runtime\a7ff19_company_numeric_confirmation_shard_$sid"
  $report = Join-Path $Repo "reports\CRYPTO_A7FF19S$sid`_COMPANY_NUMERIC_CONFIRMATION_20260530.md"
  $outLog = Join-Path $runtimeRoot "a7ff19s$sid.out.log"
  $errLog = Join-Path $runtimeRoot "a7ff19s$sid.err.log"
  $runner = Join-Path $runtimeRoot "run_a7ff19s$sid.ps1"
...
$manifestPath = Join-Path $runtimeRoot "a7ff19_company_launch_manifest.csv"
...
foreach ($row in $manifestRows) {
  $manifest = Join-Path $row.runtime ("a7ff19s" + $row.shard + "_manifest.json")
...
Write-Output "A7FF19_COMPANY_CONFIRMATION_DONE"
