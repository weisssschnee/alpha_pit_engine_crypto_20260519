...
EXTERNAL = Path("G:/AlphaFactory_CryptoData/research_runtime/a7ls10_company_numeric")
RUNTIME = REPO / "runtime" / "a7ls10_company_result_aggregate"
REPORT = REPO / "reports" / "CRYPTO_A7LS10_COMPANY_RESULT_AGGREGATE_20260606.md"
CONTRACT = REPO / "runtime" / "a7ls9_info_axis_queue_contract" / "a7ls9_manifest.json"
QUEUE = REPO / "runtime" / "a7ls9_info_axis_queue_contract" / "a7ls9_next_company_numeric_queue.csv"
...
def main() -> int:
    ...
    expected = int(contract.get("next_shard_count", 0))
    ...
    if not shortlist.empty:
        ...
        shortlist = shortlist.sort_values(["deep_followup_score", "blueprint_id"], ascending=[False, True]).head(240)

    outputs = {
        "a7ls10_shard_manifest_summary.csv": manifests,
        "a7ls10_missing_shards.csv": missing,
        "a7ls10_combined_responses.csv": responses,
        "a7ls10_combined_materialization.csv": material,
        "a7ls10_selected_portfolio_queue.csv": portfolio,
        "a7ls10_non_l7_numeric_clues.csv": non_l7,
        "a7ls10_rank_label_diagnostic_clues.csv": rank_label,
        "a7ls10_non_l7_shortlist.csv": shortlist,
        "a7ls10_non_l7_by_label.csv": summarize(non_l7, ["label_family"]),
        "a7ls10_non_l7_by_semantic_pair.csv": summarize(non_l7, ["semantic_pair"]),
        "a7ls10_non_l7_by_next_wave_family.csv": summarize(non_l7, ["next_wave_family"]),
        "a7ls10_non_l7_by_source_info_axis.csv": summarize(non_l7, ["source_info_axis"]),
        "a7ls10_non_l7_by_pair_label.csv": summarize(non_l7, ["semantic_pair", "label_family"]),
        "a7ls10_non_l7_by_shard.csv": summarize(non_l7, ["shard"]),
        "a7ls10_response_by_blocker_family.csv": summarize(responses, ["blocker_family"]),
        "a7ls10_response_by_family_blocker.csv": summarize(responses, ["next_wave_family", "blocker_family"]),
        "a7ls10_response_by_pair_blocker.csv": summarize(responses, ["semantic_pair", "blocker_family"]),
    }
    ...
    pair_summary = outputs["a7ls10_non_l7_by_semantic_pair.csv"]
    family_summary = outputs["a7ls10_non_l7_by_next_wave_family.csv"]
    axis_summary = outputs["a7ls10_non_l7_by_source_info_axis.csv"]
    ...
    decision = (
        ...
        else "HOLD_A7LS10_COMPANY_NUMERIC_NO_USABLE_CLUES"
    )
    manifest = {
        "stage": "A7LS-10-AGG",
    ...
    write_json(RUNTIME / "a7ls10_aggregate_manifest.json", manifest)
    ...
    lines = [
        ...
        "## Shard Summary",
        "",
        md_table(manifests, 80),
        "",
        "## Non-L7 By Label",
        "",
        md_table(outputs["a7ls10_non_l7_by_label.csv"]),
        "",
        "## Non-L7 By Source Info Axis",
        "",
        md_table(outputs["a7ls10_non_l7_by_source_info_axis.csv"]),
        "",
        "## Non-L7 By Next Wave Family",
        "",
        md_table(outputs["a7ls10_non_l7_by_next_wave_family.csv"]),
        "",
        "## Non-L7 By Semantic Pair",
        "",
        md_table(outputs["a7ls10_non_l7_by_semantic_pair.csv"]),
        "",
        "## Response Blockers",
        "",
        md_table(outputs["a7ls10_response_by_blocker_family.csv"]),
        "",
        "## Shortlist",
        "",
        md_table(shortlist[shortlist_cols], 80) if not shortlist.empty else "`<empty>`",
        "",
        "## Authorization",
        "",
