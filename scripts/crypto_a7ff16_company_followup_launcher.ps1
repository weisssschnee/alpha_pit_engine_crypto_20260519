...
$runtimeRoot = Join-Path $Repo "runtime\a7ff16_company_parallel"
$queuePath = Join-Path $Repo "runtime\a7ff16_cost_tiered_numeric_followup_contract\a7ff16_execution_queue.csv"
...
if (-not (Test-Path $queuePath)) {
  throw "A7FF-16 queue not found: $queuePath"
}
...
for ($i = $StartShard; $i -lt ($StartShard + $ShardCount); $i++) {
  ...
  $offset = $i * $ShardSize
  $stage = "A7FF-16S$sid"
  $filePrefix = "a7ff16s$sid"
  $runtime = Join-Path $Repo "runtime\a7ff16_company_numeric_followup_shard_$sid"
  $report = Join-Path $Repo "reports\CRYPTO_A7FF16S$sid`_COMPANY_NUMERIC_FOLLOWUP_20260530.md"
  $outLog = Join-Path $runtimeRoot "a7ff16s$sid.out.log"
  $errLog = Join-Path $runtimeRoot "a7ff16s$sid.err.log"
  $runner = Join-Path $runtimeRoot "run_a7ff16s$sid.ps1"
...
$manifestPath = Join-Path $runtimeRoot "a7ff16_company_launch_manifest.csv"
...
foreach ($row in $manifestRows) {
  $manifest = Join-Path $row.runtime ("a7ff16s" + $row.shard + "_manifest.json")
...
Write-Output "A7FF16_COMPANY_FOLLOWUP_DONE"
