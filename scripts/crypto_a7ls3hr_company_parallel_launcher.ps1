param(
  ...
  [string]$External = 'D:\HermesWorker\GDrive\AlphaFactory_CryptoData\research_runtime\a7ls3hr_company_numeric',
...
$QueuePath = Join-Path $Repo 'runtime\a7ls3hr_company_handoff_resize\a7ls3hr_company_numeric_queue_64.csv'
$AuthManifest = Join-Path $Repo 'runtime\a7ls2_sharded_materialization_wave\a7ls2_manifest.json'
...
queue = pd.read_csv(repo / 'runtime/a7ls3hr_company_handoff_resize/a7ls3hr_company_numeric_queue_64.csv')
rows_per_shard = $RowsPerShard

for shard in [$($ShardIds -join ',')]:
    part = queue.iloc[shard * rows_per_shard:(shard + 1) * rows_per_shard].copy()
    if part.empty:
        print(f's{shard:03d} rows=0 skipped_empty')
        continue
    part['a7input_queue'] = part.get('company_numeric_shard', f'a7ls3hr_s{shard:03d}')
    shard_dir = external / f'shard_{shard:03d}'
    shard_dir.mkdir(parents=True, exist_ok=True)
    part.to_csv(shard_dir / f'a7ls3hr_s{shard:03d}_queue.csv', index=False)
    print(f's{shard:03d} rows={len(part)}')
"@

$MakeQueuesPath = Join-Path $External 'make_a7ls3hr_shard_queues.py'
...
foreach ($Shard in $ShardIds) {
  ...
  $Queue = Join-Path $ShardDir "a7ls3hr_s$S`_queue.csv"
  $Manifest = Join-Path $ShardDir "a7ls3hr_s$S`_manifest.json"
  if (!(Test-Path $Queue)) {
  ...
  $Runner = Join-Path $LogDir "a7ls3hr_s$S`_$Stamp.ps1"
  $Stdout = Join-Path $LogDir "a7ls3hr_s$S`_$Stamp.stdout.log"
  $Stderr = Join-Path $LogDir "a7ls3hr_s$S`_$Stamp.stderr.log"
  $MainLog = Join-Path $LogDir "a7ls3hr_s$S`_$Stamp.main.log"
  $Report = Join-Path $ShardDir "CRYPTO_A7LS3HR_S$S`_NUMERIC_DETAIL_20260605.md"
...
$LaunchCsv = Join-Path $External "a7ls3hr_launch_$Stamp.csv"
...
Write-Output 'A7LS3HR_COMPANY_NUMERIC_DONE'
