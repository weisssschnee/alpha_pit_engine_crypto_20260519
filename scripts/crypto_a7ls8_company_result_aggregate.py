...
EXTERNAL = Path("G:/AlphaFactory_CryptoData/research_runtime/a7ls8_company_numeric")
RUNTIME = REPO / "runtime" / "a7ls8_company_result_aggregate"
REPORT = REPO / "reports" / "CRYPTO_A7LS8_COMPANY_RESULT_AGGREGATE_20260605.md"
CONTRACT = REPO / "runtime" / "a7ls7_clue_mechanism_queue_contract" / "a7ls7_manifest.json"
QUEUE = REPO / "runtime" / "a7ls7_clue_mechanism_queue_contract" / "a7ls7_next_company_numeric_queue.csv"
...
def main() -> int:
    ...
    if not shortlist.empty:
        ...
        shortlist = shortlist.sort_values(["deep_followup_score", "blueprint_id"], ascending=[False, True]).head(160)

    outputs = {
        "a7ls8_shard_manifest_summary.csv": manifests,
        "a7ls8_missing_shards.csv": missing,
        "a7ls8_combined_responses.csv": responses,
        "a7ls8_combined_materialization.csv": material,
        "a7ls8_selected_portfolio_queue.csv": portfolio,
        "a7ls8_non_l7_numeric_clues.csv": non_l7,
        "a7ls8_rank_label_diagnostic_clues.csv": rank_label,
        "a7ls8_non_l7_shortlist.csv": shortlist,
        "a7ls8_non_l7_by_label.csv": summarize(non_l7, ["label_family"]),
        "a7ls8_non_l7_by_semantic_pair.csv": summarize(non_l7, ["semantic_pair"]),
        "a7ls8_non_l7_by_next_wave_family.csv": summarize(non_l7, ["next_wave_family"]),
        "a7ls8_non_l7_by_pair_label.csv": summarize(non_l7, ["semantic_pair", "label_family"]),
        "a7ls8_non_l7_by_shard.csv": summarize(non_l7, ["shard"]),
        "a7ls8_response_by_blocker_family.csv": summarize(responses, ["blocker_family"]),
        "a7ls8_response_by_family_blocker.csv": summarize(responses, ["next_wave_family", "blocker_family"]),
        "a7ls8_response_by_pair_blocker.csv": summarize(responses, ["semantic_pair", "blocker_family"]),
    }
    ...
    pair_summary = outputs["a7ls8_non_l7_by_semantic_pair.csv"]
    family_summary = outputs["a7ls8_non_l7_by_next_wave_family.csv"]
    ...
    decision = (
        ...
        else "HOLD_A7LS8_COMPANY_NUMERIC_NO_USABLE_CLUES"
    )
    manifest = {
        "stage": "A7LS-8-AGG",
    ...
    write_json(RUNTIME / "a7ls8_aggregate_manifest.json", manifest)
    ...
    lines = [
        ...
        "## Non-L7 By Label",
        "",
        md_table(outputs["a7ls8_non_l7_by_label.csv"]),
        "",
        "## Non-L7 By Next Wave Family",
        "",
        md_table(outputs["a7ls8_non_l7_by_next_wave_family.csv"]),
        "",
        "## Non-L7 By Semantic Pair",
        "",
        md_table(outputs["a7ls8_non_l7_by_semantic_pair.csv"]),
        "",
        "## Response Blockers",
        "",
        md_table(outputs["a7ls8_response_by_blocker_family.csv"]),
        "",
        "## Shortlist",
        "",
        md_table(shortlist[shortlist_cols], 60) if not shortlist.empty else "`<empty>`",
        "",
        "## Authorization",
        "",
