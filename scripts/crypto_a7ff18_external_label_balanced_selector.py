...
RUNTIME = REPO / "runtime" / "a7ff18_external_label_balanced_selector"
REPORT = REPO / "reports" / "CRYPTO_A7FF18_EXTERNAL_LABEL_BALANCED_SELECTOR_20260530.md"
A7FF17_MANIFEST = REPO / "runtime" / "a7ff17_followup_selector_triage" / "a7ff17_manifest.json"
...
def load_materialization() -> pd.DataFrame:
    ...
    for i in range(2):
        sid = f"{i:02d}"
        p = REPO / "runtime" / f"a7ff16_company_numeric_followup_shard_{sid}" / f"a7ff16s{sid}_materialization_metrics.csv"
        if p.exists():
            df = pd.read_csv(p)
...
def main() -> None:
    ...
    a7ff17 = read_json(A7FF17_MANIFEST)
    responses = pd.read_csv(A7FF16_AGG / "a7ff16_label_response_metrics_all_shards.csv")
    mats = load_materialization()
    if responses.empty:
        raise SystemExit("missing A7FF-16 response metrics")
    if mats.empty:
        raise SystemExit("missing A7FF-16 materialization metrics")
    ...
    selected_semantic = (
        selected.groupby("semantic_pair", dropna=False).size().reset_index(name="rows").sort_values("rows", ascending=False)
        if not selected.empty
        else pd.DataFrame()
    )
    selected_motif = (
        selected.groupby("motif", dropna=False).size().reset_index(name="rows").sort_values("rows", ascending=False)
        if not selected.empty
        else pd.DataFrame()
    )
    ...
    decision = (
        ...
        and selected_unique >= 50
        and strict_rows >= 20
        and cost5_rows >= 50
        else "HOLD_A7FF18_EXTERNAL_LABEL_BALANCED_SELECTOR_INSUFFICIENT"
    )
    manifest = {
        "stage": "A7FF-18-EXTERNAL-LABEL-BALANCED-SELECTOR",
        "generated_at": now_utc(),
        "decision": decision,
        "source_a7ff17_decision": a7ff17.get("decision", ""),
        ...
        "authorizes_a7ff19_external_selector_confirmation_contract": decision.startswith("PASS"),
        "authorizes_search": False,
        "authorizes_alpha_proof": False,
        "authorizes_shadow_paper_live": False,
    }

    pool.to_csv(RUNTIME / "a7ff18_external_candidate_pool.csv", index=False)
    selected.to_csv(RUNTIME / "a7ff18_external_label_balanced_selected_queue.csv", index=False)
    candidate_label.to_csv(RUNTIME / "a7ff18_candidate_label_cost_tier_summary.csv", index=False)
    selected_label.to_csv(RUNTIME / "a7ff18_selected_label_cost_tier_summary.csv", index=False)
    selected_semantic.to_csv(RUNTIME / "a7ff18_selected_semantic_summary.csv", index=False)
    selected_motif.to_csv(RUNTIME / "a7ff18_selected_motif_summary.csv", index=False)
    write_json(RUNTIME / "a7ff18_manifest.json", manifest)

    report = f"""# CRYPTO A7FF-18 EXTERNAL LABEL-BALANCED SELECTOR
