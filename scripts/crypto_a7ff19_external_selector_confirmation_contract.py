...
RUNTIME = REPO / "runtime" / "a7ff19_external_selector_confirmation_contract"
REPORT = REPO / "reports" / "CRYPTO_A7FF19_EXTERNAL_SELECTOR_CONFIRMATION_CONTRACT_20260530.md"

A7FF18_MANIFEST = REPO / "runtime" / "a7ff18_external_label_balanced_selector" / "a7ff18_manifest.json"
A7FF18_QUEUE = REPO / "runtime" / "a7ff18_external_label_balanced_selector" / "a7ff18_external_label_balanced_selected_queue.csv"
...
def join_int_unique(values: pd.Series) -> str:
    vals = []
    for x in values.dropna().tolist():
        try:
            vals.append(int(x))
        except Exception:
            pass
...
def main() -> None:
    ...
    a7ff18 = read_json(A7FF18_MANIFEST)
    if not a7ff18.get("authorizes_a7ff19_external_selector_confirmation_contract"):
        raise SystemExit("A7FF-18 does not authorize A7FF-19 external selector confirmation")
    selected = pd.read_csv(A7FF18_QUEUE)
    if selected.empty:
        raise SystemExit("empty A7FF-18 selected queue")
    ...
    meta = (
        ...
        .agg(
            a7ff18_selected_rows=("blueprint_id", "count"),
            a7ff18_label_targets=("label_family", join_unique),
            a7ff18_horizon_targets=("label_horizon_h", join_int_unique),
            a7ff18_best_tier_rank=("tier_rank", "max"),
            a7ff18_best_selector_score=("selector_score", "max"),
            a7ff18_any_strict_cost10=("cost_tier", lambda s: bool((s == "strict_cost10").any())),
            a7ff18_any_cost5_or_better=("cost_tier", lambda s: bool(s.isin(["strict_cost10", "cost5_followup"]).any())),
        )
        .reset_index()
    )
    ...
    required_cols = [
        ...
        "motif",
        "skeleton_key",
        "a7ff18_selected_rows",
        "a7ff18_label_targets",
        "a7ff18_horizon_targets",
        "a7ff18_best_tier_rank",
        "a7ff18_best_selector_score",
        "a7ff18_any_strict_cost10",
        "a7ff18_any_cost5_or_better",
    ]
    execution_queue = execution_queue[required_cols].sort_values(
        ["a7ff18_best_tier_rank", "a7ff18_best_selector_score", "blueprint_id"],
    ...
    strict_blueprints = int(execution_queue["a7ff18_any_strict_cost10"].sum())
    cost5_blueprints = int(execution_queue["a7ff18_any_cost5_or_better"].sum())
    decision = (
        "PASS_A7FF19_EXTERNAL_SELECTOR_CONFIRMATION_CONTRACT_READY_FOR_COMPANY_EXECUTION"
        if execution_count >= 50
        and top_semantic_share <= 0.36
        and top_motif_share <= 0.36
        and strict_blueprints >= 30
        and cost5_blueprints == execution_count
        else "HOLD_A7FF19_EXTERNAL_SELECTOR_CONFIRMATION_CONTRACT_INSUFFICIENT"
    )
    ...
    runner_config = {
        ...
        "queue_local_path": str((RUNTIME / "a7ff19_execution_queue.csv").relative_to(REPO)),
        "queue_remote_path": r"runtime\a7ff19_external_selector_confirmation_contract\a7ff19_execution_queue.csv",
    ...
    manifest = {
        ...
        "decision": decision,
        "source_a7ff18_decision": a7ff18.get("decision", ""),
        "a7ff18_selected_rows": int(len(selected)),
    ...
    execution_queue.to_csv(RUNTIME / "a7ff19_execution_queue.csv", index=False)
    label_target_summary.to_csv(RUNTIME / "a7ff19_label_target_summary.csv", index=False)
    exec_semantic_summary.to_csv(RUNTIME / "a7ff19_execution_semantic_summary.csv", index=False)
    exec_motif_summary.to_csv(RUNTIME / "a7ff19_execution_motif_summary.csv", index=False)
    write_json(RUNTIME / "a7ff19_company_runner_config.json", runner_config)
    write_json(RUNTIME / "a7ff19_manifest.json", manifest)

    report = f"""# CRYPTO A7FF-19 EXTERNAL SELECTOR CONFIRMATION CONTRACT
