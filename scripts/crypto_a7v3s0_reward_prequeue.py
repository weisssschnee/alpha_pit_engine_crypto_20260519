...
DEFAULT_RUNTIME = Path(
    os.environ.get(
        "A7V3S0_PREQUEUE_RUNTIME",
        r"D:\HermesWorker\GDrive\AlphaFactory_CryptoData\research_runtime\a7v3s0_reward_prequeue_20260613",
    )
)
...
def main() -> None:
    activity_queue = Path(os.environ.get("A7V3S0_ACTIVITY_QUEUE", str(DEFAULT_ACTIVITY_QUEUE)))
    runtime = Path(os.environ.get("A7V3S0_PREQUEUE_RUNTIME", str(DEFAULT_RUNTIME)))
    target = int(os.environ.get("A7V3S0_PREQUEUE_TARGET", "1024"))
    lane_cap = int(os.environ.get("A7V3S0_PREQUEUE_LANE_CAP", str(max(1, target // 4))))
    pair_cap = int(os.environ.get("A7V3S0_PREQUEUE_PAIR_CAP", "24"))
    motif_cap = int(os.environ.get("A7V3S0_PREQUEUE_MOTIF_CAP", "160"))
    skeleton_cap = int(os.environ.get("A7V3S0_PREQUEUE_SKELETON_CAP", "8"))
    ...
    selected = bounded_select(queue, target, lane_cap, pair_cap, motif_cap, skeleton_cap)
    selected.to_csv(runtime / "a7v3s0_reward_prequeue.csv", index=False)
    ...
    lane_summary.to_csv(runtime / "a7v3s0_reward_prequeue_lane_summary.csv", index=False)
    pair_summary.to_csv(runtime / "a7v3s0_reward_prequeue_pair_summary.csv", index=False)
    motif_summary.to_csv(runtime / "a7v3s0_reward_prequeue_motif_summary.csv", index=False)

    manifest = {
        "stage": "A7V3S0-REWARD-PREQUEUE",
        ...
        "output_queue": str(runtime / "a7v3s0_reward_prequeue.csv"),
        "authorizes_reward_gate": True,
        "authorizes_alpha_proof": False,
        "authorizes_shadow_paper_live": False,
    }
    write_json(runtime / "a7v3s0_reward_prequeue_manifest.json", manifest)
