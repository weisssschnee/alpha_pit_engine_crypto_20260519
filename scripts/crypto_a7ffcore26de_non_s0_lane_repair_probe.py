...
RUNTIME = REPO / "runtime" / "a7ffcore26de_non_s0_lane_repair_probe"
REPORT = REPO / "reports" / "CRYPTO_A7FFCORE26DE_NON_S0_LANE_REPAIR_PROBE_20260602.md"
CORE26D = REPO / "runtime" / "a7ffcore26d_non_s0_lane_repair_contract" / "a7ffcore26d_manifest.json"
...
def generate_repair_pool() -> pd.DataFrame:
    ...
    specs = {
        "S3_cross_family_bridge": {
            ...
            "right_fields": ["mark_trade_basis_bps", "mark_index_basis_bps", "basis_abs_168h", "open_interest_value_last", "premium_close_bps"],
            "left_transforms": ["delta_8h", "delta_24h", "mean_24h", "decay_24h", "spread_short_long", "tsrank_168h"],
            "right_transforms": ["delta_8h", "delta_24h", "decay_24h", "decay_72h", "zscore_168h", "abs_zscore_168h", "tsrank_168h"],
            "operators": ["SafeDiv", "Mul", "Sub", "Add"],
        ...
        "S1_liquidity_basis_positioning": {
            "quota": 180,
            "left_fields": ["median_quote_volume_168h", "liquidity_rank_active_universe", "realized_vol_24h", "realized_vol_168h"],
            "right_fields": ["mark_trade_basis_bps", "mark_index_basis_bps", "premium_close_bps", "top_long_short_position_ratio_last"],
            "left_transforms": ["delta_24h", "mean_24h", "decay_24h", "zscore_168h", "tsrank_168h", "spread_short_long"],
    ...
    for lane, spec in specs.items():
        ...
        for left, lt, op, right, rt, label, horizon in product(
            ...
            blueprint_id = f"core26de_{lane}_{short_hash(expr + '|' + label + '|' + str(horizon))}"
            rows.append(
                {
...
def main() -> None:
    ...
    core26d = read_json(CORE26D)
    if core26d.get("decision") != "PASS_A7FFCORE26D_NON_S0_LANE_REPAIR_CONTRACT_READY_FOR_CORE26DE":
        raise SystemExit(f"CORE26D is not ready: {core26d.get('decision')}")
    ...
    decision = "PASS_A7FFCORE26DE_NON_S0_REPAIR_READY_FOR_CORE27_CONTRACT" if not blockers else "HOLD_A7FFCORE26DE_NON_S0_REPAIR_INSUFFICIENT"

    repair_pool.to_csv(RUNTIME / "a7ffcore26de_repair_pool.csv", index=False)
    result.to_csv(RUNTIME / "a7ffcore26de_numeric_rows.csv", index=False)
    candidate_summary.to_csv(RUNTIME / "a7ffcore26de_candidate_summary.csv", index=False)
    clean.to_csv(RUNTIME / "a7ffcore26de_clean_candidates.csv", index=False)
    near.to_csv(RUNTIME / "a7ffcore26de_near_miss_candidates.csv", index=False)
    lane_summary.to_csv(RUNTIME / "a7ffcore26de_lane_summary.csv", index=False)
    errors_df.to_csv(RUNTIME / "a7ffcore26de_eval_errors.csv", index=False)
    manifest = {
        "stage": "A7FF-CORE26DE",
        "generated_at": now_utc(),
        "source_stage": "A7FF-CORE26D",
        "source_decision": core26d.get("decision"),
        "decision": decision,
        "blockers": blockers,
        ...
        "next_allowed": "A7FF-CORE27 bounded replay contract" if decision.startswith("PASS_") else "A7FF-CORE26DER non-S0 repair forensic",
    }
    write_json(RUNTIME / "a7ffcore26de_manifest.json", manifest)
    report = [
