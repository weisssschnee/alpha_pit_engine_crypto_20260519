...
RUNTIME = REPO / "runtime" / "a7ff16_company_numeric_followup_aggregate"
REPORT = REPO / "reports" / "CRYPTO_A7FF16_COMPANY_NUMERIC_FOLLOWUP_AGGREGATE_20260530.md"
...
def shard_dir(i: int) -> Path:
    return SHARD_ROOT / f"a7ff16_company_numeric_followup_shard_{i:02d}"
...
def main() -> None:
    ...
    clue_rows = (
        responses[responses.get("decision", pd.Series(dtype=str)).astype(str).str.contains("NUMERIC_CLUE", na=False)].copy()
        if not responses.empty
        else pd.DataFrame()
    )
    non_l7 = (
        clue_rows[~clue_rows.get("label_family", pd.Series(dtype=str)).eq("L7_ranked_future_return")].copy()
    ...
    decision = (
        ...
        else "HOLD_A7FF16_COMPANY_NUMERIC_FOLLOWUP_INCOMPLETE_OR_EMPTY"
    )
    manifest = {
    ...
    shards.to_csv(RUNTIME / "a7ff16_shard_summary.csv", index=False)
    responses.to_csv(RUNTIME / "a7ff16_label_response_metrics_all_shards.csv", index=False)
    clue_summary.to_csv(RUNTIME / "a7ff16_non_l7_clue_summary.csv", index=False)
    label_summary.to_csv(RUNTIME / "a7ff16_label_summary.csv", index=False)
    selected.to_csv(RUNTIME / "a7ff16_selected_portfolio_queue_all_shards.csv", index=False)
    decisions.to_csv(RUNTIME / "a7ff16_decision_counts_all_shards.csv", index=False)
    write_json(RUNTIME / "a7ff16_manifest.json", manifest)

    report = f"""# CRYPTO A7FF-16 COMPANY NUMERIC FOLLOWUP AGGREGATE
