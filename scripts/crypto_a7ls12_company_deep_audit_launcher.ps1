param(
  ...
  [string]$External = 'D:\HermesWorker\GDrive\AlphaFactory_CryptoData\research_runtime\a7ls12_company_deep_audit',
...
$QueuePath = Join-Path $Repo 'runtime\a7ls12_deep_audit_packet\a7ls12_deep_audit_queue.csv'
$AuthManifest = Join-Path $Repo 'runtime\a7ls12_deep_audit_packet\a7ls12_manifest.json'
...
queue = pd.read_csv(repo / 'runtime/a7ls12_deep_audit_packet/a7ls12_deep_audit_queue.csv')

for shard in [$($ShardIds -join ',')]:
    shard_name = f'a7ls12_s{shard:03d}'
    part = queue.loc[queue['a7ls12_deep_shard'].astype(str).eq(shard_name)].copy()
    if part.empty:
    ...
    part.to_csv(shard_dir / f'a7ls12_s{shard:03d}_queue.csv', index=False)
    print(f'{shard_name} rows={len(part)}')
"@

$MakeQueuesPath = Join-Path $External 'make_a7ls12_shard_queues.py'
...
foreach ($Shard in $ShardIds) {
  ...
  $Queue = Join-Path $ShardDir "a7ls12_s$S`_queue.csv"
  $Manifest = Join-Path $ShardDir "a7ls12_s$S`_manifest.json"
  if (!(Test-Path $Queue)) {
  ...
  $Runner = Join-Path $LogDir "a7ls12_s$S`_$Stamp.ps1"
  $Stdout = Join-Path $LogDir "a7ls12_s$S`_$Stamp.stdout.log"
  $Stderr = Join-Path $LogDir "a7ls12_s$S`_$Stamp.stderr.log"
  $MainLog = Join-Path $LogDir "a7ls12_s$S`_$Stamp.main.log"
  $Report = Join-Path $ShardDir "CRYPTO_A7LS12_S$S`_DEEP_AUDIT_20260606.md"
...
$LaunchCsv = Join-Path $External "a7ls12_launch_$Stamp.csv"
...
Write-Output 'A7LS12_COMPANY_DEEP_AUDIT_DONE'
