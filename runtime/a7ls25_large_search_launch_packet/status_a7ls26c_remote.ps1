...
$queueSummary = Join-Path $Root "a7ls26c_numeric_queue_summary.json"
...
if (Test-Path (Join-Path $Root "shards")) {
  Get-ChildItem (Join-Path $Root "shards") -Directory |
    ForEach-Object {
      $sid = $_.Name
      $manifest = Join-Path $_.FullName ("a7ls26c_" + $sid + "_manifest.json")
...
Write-Host "== SUMMARY =="
$summary = Join-Path $Root "a7ls26c_numeric_wave_summary.json"
