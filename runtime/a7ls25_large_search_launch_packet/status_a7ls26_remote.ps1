...
$Root = "D:\HermesWorker\GDrive\AlphaFactory_CryptoData\research_runtime\a7ls26_numeric_wave_20260608"
...
$queueSummary = Join-Path $Root "a7ls26_numeric_queue_summary.json"
if (Test-Path $queueSummary) {
  Get-Content $queueSummary -Raw
} else {
  Write-Host "queue summary not ready"
}
...
if (Test-Path (Join-Path $Root "shards")) {
  Get-ChildItem (Join-Path $Root "shards") -Directory |
    ForEach-Object {
      $sid = $_.Name
      $manifest = Join-Path $_.FullName ("a7ls26_" + $sid + "_manifest.json")
...
Write-Host "== SUMMARY =="
$summary = Join-Path $Root "a7ls26_numeric_wave_summary.json"
if (Test-Path $summary) {
  Get-Content $summary -Raw
} else {
  Write-Host "numeric wave summary not ready"
}
