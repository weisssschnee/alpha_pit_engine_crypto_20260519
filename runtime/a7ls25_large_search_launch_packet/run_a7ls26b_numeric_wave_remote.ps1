...
$Runtime = "D:\HermesWorker\GDrive\AlphaFactory_CryptoData\research_runtime\a7ls26b_numeric_wave_20260608"
$Queue = Join-Path $Runtime "a7ls26b_numeric_queue_8192.csv"
$Concurrency = 2
$RowsPerShard = 64
$TargetRows = 8192
...
$AggregateScript = Join-Path $Runtime "build_a7ls26b_numeric_queue.py"
@'
...
runtime = Path(r"D:\HermesWorker\GDrive\AlphaFactory_CryptoData\research_runtime\a7ls26b_numeric_wave_20260608")
target_rows = 8192

source_queue = pd.read_csv(mat_root / "a7ls25_materialization_queue_40k.csv", low_memory=False)
used_ids = set()
prev_queue = prev_runtime / "a7ls26_numeric_queue_4096.csv"
if prev_queue.exists():
    used_ids = set(pd.read_csv(prev_queue, usecols=["blueprint_id"])["blueprint_id"].astype(str))
...
queue.to_csv(runtime / "a7ls26b_numeric_queue_8192.csv", index=False)

summary = {
    "stage": "A7LS-26B",
    "decision": "PASS_A7LS26B_NUMERIC_QUEUE_BUILT",
    "excluded_previous_a7ls26_rows": int(len(used_ids)),
...
(runtime / "a7ls26b_numeric_queue_summary.json").write_text(json.dumps(summary, indent=2, sort_keys=True), encoding="utf-8")
queue.groupby(["a7ls25_axis", "semantic_pair", "motif"], dropna=False).size().reset_index(name="rows").sort_values("rows", ascending=False).to_csv(runtime / "a7ls26b_numeric_queue_family_summary.csv", index=False)
print(json.dumps(summary, indent=2, sort_keys=True))
'@ | Set-Content -Encoding UTF8 $AggregateScript

& $Python $AggregateScript
if ($LASTEXITCODE -ne 0) {
  throw "A7LS26B queue build failed with exit code $LASTEXITCODE"
}

$shards = @()
for ($start = 0; $start -lt $TargetRows; $start += $RowsPerShard) {
  $end = [Math]::Min($start + $RowsPerShard, $TargetRows)
  $sid = "a7ls26b_num_s{0:D3}" -f ($start / $RowsPerShard)
  $shards += [pscustomobject]@{ id = $sid; start = $start; end = $end }
}

$active = @()
foreach ($s in $shards) {
  $manifest = Join-Path $Runtime ("shards\" + $s.id + "\a7ls26b_" + $s.id + "_manifest.json")
  if (Test-Path $manifest) {
    Write-Host "[A7LS26B] skip existing $($s.id)"
    continue
  }
  ...
  $env:A7FF8_STAGE = "A7LS-26B-" + $s.id
  $env:A7FF8_FILE_PREFIX = "a7ls26b_" + $s.id
  $env:A7FF8_RUNTIME = $shardRoot
  $env:A7FF8_REPORT = Join-Path $shardRoot ("A7LS26B_" + $s.id + "_REPORT.md")
  ...
  Write-Host "[A7LS26B] start $($s.id) rows=$($s.start):$($s.end)"
...
$summary | ConvertTo-Json -Depth 5 | Set-Content -Encoding UTF8 (Join-Path $Runtime "a7ls26b_numeric_wave_summary.json")
Write-Host "[A7LS26B] numeric wave complete"
