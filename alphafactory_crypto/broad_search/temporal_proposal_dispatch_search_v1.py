"""One bounded train-only targeted search using Proposal Dispatcher V1."""
...
EXECUTION_MODE = "SEARCH_CORE_PROPOSAL_DISPATCH_SUCCESSOR_V1"
CAMPAIGN = "crypto_temporal_proposal_dispatch_successor_v1"
AUTHORIZATION_PATH = "config/crypto_temporal_proposal_dispatch_successor_v1_authorization.json"
HISTORICAL_PRIOR_PATH = "config/crypto_temporal_proposal_dispatch_v1_historical_prior.json"
STRICT_CAP = 20_000
CHECKPOINT_SIZE = 2_000
RAW_ATTEMPT_CAP = 750_000
WORKERS = 8
LANE_COUNT = 4
REQUIRED_EXECUTION_COMPONENT_PATHS = (
    ...
    "scripts/analyze_crypto_temporal_proposal_dispatch_v1.py",
    "scripts/check_crypto_temporal_proposal_dispatch_v1.py",
    "scripts/run_crypto_temporal_proposal_dispatch_v1.py",
    "scripts/run_crypto_temporal_proposal_dispatch_v1_pc2.ps1",
)
...
def validate_authorization(repo_root: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or payload.get("status") != "RUN_AUTHORIZED_ONE_TIME_PROPOSAL_DISPATCH_20000"
...
def _make_policies(
    *, registry: Any, config: Mapping[str, Any], catalog: Sequence[Any],
    ...
    for seed in LANE_SEEDS:
        ...
        configure_policy_dispatcher_v1(policy, historical_prior=prior)
...
def _write_final(
    ...
    result = {
        "schema_version": 1,
        "status": "PROPOSAL_DISPATCH_SUCCESSOR_20000_COMPLETE",
        "strict": len(ledger),
...
def _execute(
    ...
    inventory: Any, prior: Mapping[str, Any], executor: concurrent.futures.ProcessPoolExecutor,
) -> dict[str, Any]:
            ...
            while len(proposals) < WORKERS and len(ledger) + len(proposals) < target:
                ...
                try:
                    candidate, metadata = propose_with_dispatcher_v1(
                        policy, scale_contract=config["time_scale_authority"], inventory=inventory,
                    )
...
def run(repo_root: Path, *, runtime_id: str) -> dict[str, Any]:
    ...
    claim = {"schema_version": 1, "status": "ONE_TIME_PROPOSAL_DISPATCH_SEARCH_LAUNCHED",
    ...
    with concurrent.futures.ProcessPoolExecutor(
        ...
        return _execute(runtime_root, source_sha=source_sha, frozen_hash=frozen_hash,
                        ...
                        pool=pool, baseline=baseline, inventory=inventory, prior=prior, executor=executor)
