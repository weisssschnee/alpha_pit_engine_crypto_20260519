...
Generated from `config/crypto_architecture_control_registry_v1.json`. Registry SHA256: `DFCB3AC1B73CEC0474B937CB77A7E3C7CD275D15D011A5D586C0C57B690AAC18`.

Status: `EPOCH1R_ADMISSION_PREFLIGHT_PASSED_READY_TO_FREEZE` / `EPOCH1R_ADMISSION_PREFLIGHT_PASSED_READY_TO_FREEZE` / `EPOCH1_EXECUTION_FAILED_PRE_STRICT` / `FROZEN_DEVELOPMENT_EPOCH_COMPLETED` / `NEXTGEN_DARK_SCOPED_READY` / `B1S_CANARY_COMPLETED_WITH_NATURAL_QUOTA_UNDERFILL` / `PRODUCTION_OBSERVATION_PARTIALLY_QUALIFIED` / `FORMAL_SEARCH_FROZEN` / `ADAPTIVE_CROSS_EPOCH_MEMORY_FROZEN` / `NO_CANDIDATE_PROMOTION` / `SEALED_NO_NEW_FORWARD_READ`.
...
| CRYPTO NEXTGEN SEARCH EPOCH-1R frozen design | PLANNED | scripts/crypto_nextgen_epoch1r.py | crypto_nextgen_epoch1r.py freeze | committed proposal pack, capacity table, assignments, repair code and unchanged Epoch-1 contracts -> new immutable Epoch-1R manifest | frozen repaired development-search control | NO_STRICT_BEFORE_PREFLIGHT_AND_FREEZE | tests/test_nextgen_epoch1r.py; runtime/nextgen_epoch1r_20260712/proposal_pack.jsonl.gz; runtime/nextgen_epoch1r_20260712/full_identity_records.jsonl.gz; runtime/nextgen_epoch1r_20260712/epoch1r_test_output.txt | 26CD72173720FA417870A05415BB22CCB7B440D4868A09FC757C2BC37A5E54BE | immutable pack/preflight artifacts ready; new Epoch-1R manifest not yet frozen |
| CRYPTO NEXTGEN SEARCH EPOCH-1R execution | PLANNED | scripts/crypto_nextgen_epoch1r.py | crypto_nextgen_epoch1r.py run/check | frozen Epoch-1R assignment and development-only panels -> strict evaluations and truthful stopped recommendation | controlled development-only strict evaluation | CURRENT_RUNTIME_ONLY_NO_OOS_NO_FORWARD_NO_MEMORY_NO_PROMOTION | tests/test_nextgen_epoch1r.py | 940C57D93B6EBC10834A47F369438380D7DFD6D89912EDB9957B6F1378938ADA | Epoch-1R is not frozen or started |
