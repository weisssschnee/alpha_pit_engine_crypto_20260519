...
Generated from `config/crypto_architecture_control_registry_v1.json`. Registry SHA256: `B5D5BB8297C01EC9F74F72B00B2A456B2885A4F6B256B485EBB9EB4E2CE30355`.

Status: `EPOCH2_CALIBRATION_AND_TAXONOMY_QUALIFIED` / `EPOCH2_CALIBRATION_AND_TAXONOMY_QUALIFIED` / `FROZEN_DEVELOPMENT_EPOCH1R_COMPLETED_WITH_NATURAL_UNDERFILL` / `EPOCH1_EXECUTION_FAILED_PRE_STRICT` / `FROZEN_DEVELOPMENT_EPOCH_COMPLETED` / `PRODUCTION_OBSERVATION_PARTIALLY_QUALIFIED` / `FORMAL_SEARCH_FROZEN` / `ADAPTIVE_CROSS_EPOCH_MEMORY_FROZEN` / `NO_CANDIDATE_PROMOTION` / `SEALED_NO_NEW_FORWARD_READ`.
...
| Epoch-2 blocker-directed search | PLANNED | config/crypto_epoch2_v1.json | planned Epoch-2 runner | frozen parent pack and fresh typed roots -> 49152 fixed proposals and repair lineage | development-only proposal search | CURRENT_EPOCH_ONLY_NO_CROSS_EPOCH_POLICY_OR_VALUE | planned | B444CE28AF8649924C3C14BE1776771A050F2E7B0AFEC2A146A9A04D6957AC4F | runner and tests pending |
| CRYPTO EPOCH-2 frozen design | PLANNED | config/crypto_epoch2_v1.json | planned freeze | calibration, taxonomy, actions, budgets, policies, seeds and evaluator hash -> immutable Epoch-2 manifest | frozen development-search control | NO_PERFORMANCE_BEFORE_FREEZE | planned | B444CE28AF8649924C3C14BE1776771A050F2E7B0AFEC2A146A9A04D6957AC4F | search implementation and freeze pending |
| CRYPTO EPOCH-2 execution | PLANNED | config/crypto_epoch2_v1.json | planned run/check | frozen 49152 proposals and three equal-budget admission policies -> 2304 strict evaluations and stopped recommendation | controlled development-only evidence | NO_OOS_NO_FORWARD_NO_MEMORY_NO_PROMOTION | planned | B444CE28AF8649924C3C14BE1776771A050F2E7B0AFEC2A146A9A04D6957AC4F | not frozen or started |
