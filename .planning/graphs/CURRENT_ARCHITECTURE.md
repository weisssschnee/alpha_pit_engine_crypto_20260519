...
Generated from `config/crypto_architecture_control_registry_v1.json`. Registry SHA256: `8BCFB60BD0941E8C60236A2DBCA7E2776040CA184AD09CA7FFA644EE988301DF`.

Status: `EPOCH2_STRICT_EXECUTION_COMPLETE_POSTPROCESS_FAILED` / `EPOCH2_STRICT_EXECUTION_COMPLETE_POSTPROCESS_FAILED` / `FROZEN_DEVELOPMENT_EPOCH1R_COMPLETED_WITH_NATURAL_UNDERFILL` / `EPOCH1_EXECUTION_FAILED_PRE_STRICT` / `FROZEN_DEVELOPMENT_EPOCH_COMPLETED` / `PRODUCTION_OBSERVATION_PARTIALLY_QUALIFIED` / `FORMAL_SEARCH_FROZEN` / `ADAPTIVE_CROSS_EPOCH_MEMORY_FROZEN` / `NO_CANDIDATE_PROMOTION` / `SEALED_NO_NEW_FORWARD_READ`.
...
| CRYPTO EPOCH-2 execution | PARTIAL | config/crypto_epoch2_v1.json; scripts/crypto_epoch2.py; scripts/crypto_epoch2_postprocess_repair.py | python scripts/crypto_epoch2.py run; python scripts/crypto_epoch2.py check | frozen 49152 proposals and three equal-budget admission policies -> 2304 strict evaluations and stopped recommendation | controlled development-only evidence | NO_OOS_NO_FORWARD_NO_MEMORY_NO_PROMOTION | tests/test_epoch2.py; tests/test_epoch2_postprocess_repair.py; runtime/epoch2_20260712/epoch2_implementation_test_output.txt; runtime/epoch2_20260712/proposal_pack.jsonl.gz; runtime/epoch2_20260712/admission_assignments.csv; runtime/epoch2_20260712/strict_evaluations.csv; runtime/epoch2_20260712/epoch2_failure.json | B79CE62528944B325AF0338D08F87EE1AB9249C57286FBEBDDFD04F532C53D7D | 49152 proposals and 2304 strict rows complete; report-only proposal_id schema recovery pending with zero new performance queries |
