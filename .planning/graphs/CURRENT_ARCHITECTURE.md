...
Generated from `config/crypto_architecture_control_registry_v1.json`. Registry SHA256: `27A2B09389855011B96FA065D375B6D339C8588C4EC840C98CFDE96A5A43FF0C`.

Status: `EPOCH2_BLOCKER_DIRECTED_SEARCH_IMPLEMENTATION_READY` / `EPOCH2_BLOCKER_DIRECTED_SEARCH_IMPLEMENTATION_READY` / `FROZEN_DEVELOPMENT_EPOCH1R_COMPLETED_WITH_NATURAL_UNDERFILL` / `EPOCH1_EXECUTION_FAILED_PRE_STRICT` / `FROZEN_DEVELOPMENT_EPOCH_COMPLETED` / `PRODUCTION_OBSERVATION_PARTIALLY_QUALIFIED` / `FORMAL_SEARCH_FROZEN` / `ADAPTIVE_CROSS_EPOCH_MEMORY_FROZEN` / `NO_CANDIDATE_PROMOTION` / `SEALED_NO_NEW_FORWARD_READ`.
...
| CRYPTO EPOCH-2 frozen design | PLANNED | config/crypto_epoch2_v1.json; scripts/crypto_epoch2.py | python scripts/crypto_epoch2.py freeze | calibration, taxonomy, actions, budgets, policies, seeds and evaluator hash -> immutable Epoch-2 manifest | frozen development-search control | NO_PERFORMANCE_BEFORE_FREEZE | planned | 7D22F503753FDB31FE43D07ECF58707113BBB57C246CB2439E91AF93B8EB237D | implementation commit and immutable freeze manifest pending |
| CRYPTO EPOCH-2 execution | PLANNED | config/crypto_epoch2_v1.json; scripts/crypto_epoch2.py | python scripts/crypto_epoch2.py run; python scripts/crypto_epoch2.py check | frozen 49152 proposals and three equal-budget admission policies -> 2304 strict evaluations and stopped recommendation | controlled development-only evidence | NO_OOS_NO_FORWARD_NO_MEMORY_NO_PROMOTION | tests/test_epoch2.py; runtime/epoch2_20260712/epoch2_implementation_test_output.txt | B6359AD33CF97F75522C25035BAAAA1E29415EC59B7470AEADC1E0F956E9682A | immutable Epoch-2 design not yet frozen; performance not started |
