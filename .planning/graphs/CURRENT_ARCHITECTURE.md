...
Generated from `config/crypto_architecture_control_registry_v1.json`. Registry SHA256: `92742C026BADF15834841999AB85C2D4F07F452B6BAD5C3719D38DDC9830A67D`.

Status: `EPOCH1R_ADMISSION_REPAIR_IMPLEMENTED_PREFLIGHT_PENDING` / `REPAIR_EMPTY_ADMISSION_IMPLEMENTED_PREFLIGHT_PENDING` / `EPOCH1_EXECUTION_FAILED_PRE_STRICT` / `FROZEN_DEVELOPMENT_EPOCH_COMPLETED` / `NEXTGEN_DARK_SCOPED_READY` / `B1S_CANARY_COMPLETED_WITH_NATURAL_QUOTA_UNDERFILL` / `PRODUCTION_OBSERVATION_PARTIALLY_QUALIFIED` / `FORMAL_SEARCH_FROZEN` / `ADAPTIVE_CROSS_EPOCH_MEMORY_FROZEN` / `NO_CANDIDATE_PROMOTION` / `SEALED_NO_NEW_FORWARD_READ`.
...
| NEXTGEN isolated search engine | IMPLEMENTED | alphafactory_crypto/nextgen_epoch.py; scripts/crypto_nextgen_epoch0.py; config/crypto_nextgen_epoch0_v1.json | crypto_nextgen_epoch0 smoke/freeze/run/check | frozen mechanism registry, lane roots, seeds, semantic-volume and admission contracts -> typed random/AST, CEM, multi-step UCT/MCTS, evolutionary, surrogate, LLM-repair and orthogonal proposals | development-only proposal and lane-local adaptation | CURRENT_EPOCH_LANE_SEED_LOCAL_ONLY_NO_PERSISTENCE | tests/test_nextgen_epoch.py; tests/test_nextgen_epoch_runner.py | D9E6C8A11EBBCDAEEC2E7B238127F5479C0B38E9962E27660FEA6FD1A5717E8C | Epoch-0 completed but adaptive lanes did not improve survivor production; UCT reward basin concentrated |
...
| Epoch-1R empty-admission repair | IMPLEMENTED | alphafactory_crypto/search_revision.py; scripts/crypto_nextgen_epoch1r.py | normalize_admission_rows / admit_full_identity / crypto_nextgen_epoch1r.py pack/preflight | unchanged Epoch-1 proposal contract and possibly empty representative sets -> standard-schema zero-capacity natural underfill and isolated Epoch-1R workflow | narrow deterministic admission repair | NO_NEW_PERFORMANCE_CONCLUSION_REPAIR_AND_PREFLIGHT_ONLY | tests/test_search_revision.py; tests/test_nextgen_epoch1r.py | 9EA0AF38E473353C8614E7B2E1DF8818FA255BC526DF49C90198626190A4F4B4 | full 32768-proposal admission-only preflight not yet executed |
| CRYPTO NEXTGEN SEARCH EPOCH-1R frozen design | PLANNED | scripts/crypto_nextgen_epoch1r.py | crypto_nextgen_epoch1r.py freeze | committed proposal pack, capacity table, assignments, repair code and unchanged Epoch-1 contracts -> new immutable Epoch-1R manifest | frozen repaired development-search control | NO_STRICT_BEFORE_PREFLIGHT_AND_FREEZE | tests/test_nextgen_epoch1r.py | 7FC91FF46EF8AA1571A11DFC444075FA842E2C84B24DE3B5216D0D61A7158E4F | proposal pack and admission-only preflight pending |
| CRYPTO NEXTGEN SEARCH EPOCH-1R execution | PLANNED | scripts/crypto_nextgen_epoch1r.py | crypto_nextgen_epoch1r.py run/check | frozen Epoch-1R assignment and development-only panels -> strict evaluations and truthful stopped recommendation | controlled development-only strict evaluation | CURRENT_RUNTIME_ONLY_NO_OOS_NO_FORWARD_NO_MEMORY_NO_PROMOTION | tests/test_nextgen_epoch1r.py | 7FC91FF46EF8AA1571A11DFC444075FA842E2C84B24DE3B5216D0D61A7158E4F | Epoch-1R is not frozen or started |
