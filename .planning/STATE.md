...
Last updated: 2026-07-27 Asia/Hong_Kong

## Current phase

`CRYPTO_SEARCH_ENGINE_V1_POST_AUDIT_REMEDIATION_COMPLETED`

The Search Engine V1 post-audit remediation is complete at `369dbb8fadf7a1308fd1820da37e5cd95ffc8450`. The historical Broad 39 campaign remains immutable and independently passes its 20,000-candidate, ten-checkpoint, receipt, replay, and artifact-integrity checks. Its research and future-Arena component qualification is now HOLD because partition-local panel context polluted behavior identity, the overlapping 4h reward LCB used iid hourly uncertainty, and the ledger omitted the full matched economic waterfall. New caches fail closed unless all three context fields are rebuilt after the joined panel; the evaluator now uses horizon-aware HAC, future ledgers retain the full primary/control/incremental monthly gross/cost/net/turnover waterfall, CPU-hour covers parent and worker process CPU, valid exact-unique density uses its actual counter, and checkpoint restore verification precedes atomic publication. No cache rebuild, rescue rerun, new-data Arena, OOS, challenge, promotion, or sealed read was performed.
...
Keep the historical Search Engine V1 bundle immutable and its component qualification on HOLD. Use `surface_backfill_requirements.parquet` to close the 1,198 exact historical symbol-surface gaps, obtain authoritative `BDXNUSDT` and inferred-lifecycle identities, and materialize the Top50 OI/mark consumer. Rebuild the full Broad39 post-join schema-2 cache only after those gates pass, then seek separate authorization for any fresh-state requalification Arena. Do not rerun spent evidence, start another current-data search, train latent priority, open sealed roles, or promote candidates.
